	...
	DS.css.innerHTML = 'body { font: bold 12pt monospace; margin: 0; overflow: hidden; }' +
		...
		'iframe #overview { border: red solid 3px; opacity: 0.2; }' +
	'';	
	...
	DS.watermark = document.body.appendChild( document.createElement( 'div' ) );
	DS.watermark.style.cssText = DS.basic + DS.height + DS.horzRdm + DS.top + 'font-size: 48pt; margin-top: 200px; opacity: 0.08; text-align:center; transform:rotate(7deg); -webkit-transform:rotate(-10deg);';	
	...
	DS.displayMarkdown = function( fname, element ) {
		...
		if ( window.frames.length > 0 ) {
			DS.watermark.innerHTML = 'Working Sample';
		} else {
			DS.watermark.innerHTML = '';
		}		
